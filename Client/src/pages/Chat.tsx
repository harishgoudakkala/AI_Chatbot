  ...
  return (
   <Box sx={{display:"flex", flex:1, width:'100%', height:"100%", mt:3, gap:3}}>
      <Box sx={{display:{md:"flex", xs:"none", sm:"none"}, flex:0.2, flexDirection:"column" }}>
        <Box sx={{display:"flex", width:"100%", height:"60vh",bgcolor:"rgb(17,29,39)", borderRadius:5, flexDirection:"column", mx:3,  padding:"10% 0"}}>
            <Avatar sx={{mx:"auto", my:"2", bgcolor:'white', color:'black', fontWeight:700, mb:2}}>{ auth?.user?.name?.[0] }{auth?.user?.name?.split(" ")[1]?.[0]}</Avatar>
      ...
      <Box sx={{display:"flex", flex:{md:0.8, xs:1, sm:1}, flexDirection:"column", px:3 }}>
        <Typography sx={{textAlign:'center', fontSize:"40px" ,color:"white", mb:"20px", mx:"auto"}}> Model - Gemini-1.5-Flash</Typography>
